...
2. [Configuration](docs/src/configuration.md) → [Syncing chain config](docs/src/config-sync.md)
3. [Deploying](docs/src/deploy.md) → [Configuring the contracts](docs/src/configure.md)
4. [Full flow example](docs/src/full-flow.md) — end-to-end commands once the above make sense
5. [Governance checks](docs/src/governance.md), [Handover](docs/src/handover.md), [Safe batches](docs/src/safe-batches.md)
...
Then follow the [documentation reading order](#documentation) above — do not skip straight
to the [full flow](docs/src/full-flow.md) until config, deploy and configure are clear.

## Testing

```bash
make test          # hermetic; no RPC
make test-config   # offline config-sync selftest
