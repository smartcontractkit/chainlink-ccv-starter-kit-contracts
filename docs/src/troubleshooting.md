...
| dry-run `DeployVerifier`, then real `deploy-verifier` same tag | record already has tag | **`DeployVerifier: versionTag … already recorded`** — delete `config/deployments/<alias>.json` or `ALLOW_TAG_REPLACE=true` |
| dry-run any deploy, chain still empty | record points at addresses with no code | `deploy-resolver` or configure: **`no code at …`** |
| simulated success log but tx failed on broadcast | file may hold sim-only state | check explorer; fix record or delete and re-deploy |
