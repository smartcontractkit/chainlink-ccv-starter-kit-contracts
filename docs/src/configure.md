...
forge script script/configure/ApplySignatureConfigs.s.sol \
  --sig "run(string,bytes4)" base_sepolia $TAG --rpc-url $BASE_SEPOLIA_RPC_URL --broadcast --aws
```
...
forge script script/configure/SetDynamicConfig.s.sol \
  --sig "run(string,bytes4)" sepolia $TAG --rpc-url $SEPOLIA_RPC_URL --broadcast --aws

forge script script/configure/SetAllowedFinalityConfig.s.sol \
  --sig "run(string,bytes4)" sepolia $TAG --rpc-url $SEPOLIA_RPC_URL --broadcast --aws

forge script script/configure/UpdateStorageLocations.s.sol \
  --sig "run(string,bytes4)" sepolia $TAG --rpc-url $SEPOLIA_RPC_URL --broadcast --aws
```
