...
render() {
    ...
    if [ -z "$names" ]; then
        ...
        echo "re-run \`make deployments-doc\` to regenerate this page."
        return 0
    fi

    echo "> Generated by \`make deployments-doc\` from \`config/\`."
    echo "> Do not edit by hand — re-run \`make deployments-doc\` after a deploy."
    echo
