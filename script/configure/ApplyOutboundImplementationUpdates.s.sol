...
contract ApplyOutboundImplementationUpdates is BaseScript {
  ...
  function callsFor(address resolver, VersionedVerifierResolver.OutboundImplementationArgs[] memory args)
    public
    pure
  ...
  function run(string calldata chainAlias) external {
    ...
    require(dep.resolver != address(0), string.concat("ApplyOutboundImplementationUpdates: resolver not recorded for ", chainAlias));
    require(dep.verifier != address(0), string.concat("ApplyOutboundImplementationUpdates: verifier not recorded for ", chainAlias));
    ...
    VersionedVerifierResolver.OutboundImplementationArgs[] memory args = _buildOutboundArgs(lanePaths, chainAlias, dep.verifier);
  ...
  function _buildOutboundArgs(string[] memory lanePaths, string memory chainAlias, address verifier)
    internal
    view
